    ...
    try {
      ...
      switch (message.command) {
        ...
        case "generateCSS":
          ...
          outputChannel?.appendLine(
            `📄 Settings received: ${JSON.stringify(message.settings, null, 2)}`
          )
        ...
        case "setLayerAlias": {
          try {
            ...
            // Функція очищення імен файлів для універсального використання
            function sanitizeFileName(name) {
              return name
                .toLowerCase()
                .replace(/[^a-z0-9#-]/g, "-") // Дозволяємо літери, цифри, # та дефіси
            ...
            // Фронтенд сам обробить збереження CSS файлу
            panel.webview.postMessage({
              command: "aliasSet",
              success: true,
              layerId,
              newName,
              oldName,
              autoSave: true // Вказуємо фронтенду що потрібно зберегти файл
            })
          } catch (e) {
      ...
      if (settings.figmaLink && settings.figmaToken) {
        ...
        if (fileId) {
          ...
          const selectedCanvasIds = Array.isArray(settings.selectedCanvases)
            ...
            : []
          const selectedLayerIds = Array.isArray(settings.selectedLayers)
            ...
            try {
              const filterFigmaDoc = doc => {
                ...
                const filterByLayers = node => {
                  ...
                  const filteredKids = kids.map(child => filterByLayers(child)).filter(Boolean)
                  const selfSelected = selectedLayerIds.includes(node.id)
                  // Якщо обрано саме цей вузол як Root — зберігаємо ВСЕ його піддерево без фільтрації
                  if (selfSelected) {
                    return {...node, children: kids}
                  }
                  // Якщо серед нащадків є вибрані — залишаємо тільки відфільтрованих дітей
                  if (filteredKids.length > 0) {
                    return {...node, children: filteredKids}
                  }
                  // Якщо список selectedLayerIds порожній — повертаємо вузол як є
                  if (selectedLayerIds.length === 0) {
                    return node
                  }
                  // Інакше відсікаємо гілку
                  return null
                }

                const newPages = pageFilter
                  .map(p => {
              ...
              outputChannel?.appendLine(
                `✅ Applied Figma filters: canvases=${selectedCanvasIds.length}, layers=${selectedLayerIds.length}`
              )
            } catch (e) {
          ...
          const cssGenerator = new AdvancedCSSGenerator({
            ...
            includeGlobal: settings.globalStylesToggle === true,
            includeCustom: settings.customStylesToggle === true,
            mode: settings.mode,
...
// =======================================
// 🔤 ВСТАВКА ПОСИЛАННЯ НА ШРИФТИ У HTML
// =======================================
async function addFontsLinkToHTML(htmlFilePath, cssFilePath) {
  try {
    if (!htmlFilePath || !cssFilePath) return
    const fsP = require("fs").promises
    const pathM = require("path")
    let htmlContent = await fsP.readFile(htmlFilePath, "utf8")

    // If already linked, skip
    if (htmlContent.includes("<!--!!! FONTS !!!-->") || htmlContent.includes(cssFilePath)) return

    const relHref = pathM.relative(pathM.dirname(htmlFilePath), cssFilePath).replace(/\\/g, "/")
    const comment = "<!--!!! FONTS !!!-->"
    const linkTag = `<link rel="stylesheet" href="${relHref}">`
    ...
    if (viewportMeta) {
      ...
      outputChannel?.appendLine("⚠️ Could not find suitable place to insert FONTS link")
      return
    }
    ...
    outputChannel?.appendLine("✅ Fonts link injected into HTML head")
  } catch (e) {
    outputChannel?.appendLine(`⚠️ Could not inject fonts link: ${e.message}`)
  }
}
...
function inferStylesFromSelector(selector) {
  try {
    ...
    // клас або тег
    if (last.startsWith(".")) {
      return getHeuristicStylesForClass(last.slice(1))
    }
    if (last.startsWith("#")) {
      return ["display: block;"]
    }
    // тег
    return getHeuristicStylesForTag(last.toLowerCase())
  } catch (_) {
    return []
  }
}
...
async function handleImportFonts(panel, message) {
  try {
    ...
    const outputDir = defaultDir
    const outputFileName = `fonts-${timestamp}`
    ...
    // Додаємо імпорти Google Fonts або лінк на CSS і ставимо фокус на HTML
    const cssFilePath = result.cssFile || path.join(outputDir, `${outputFileName}.css`)
    if (htmlPath) {
      try {
        if (result && result.imports && result.imports.html) {
          await addFontsImportsToHTML(htmlPath, result.imports.html)
        } else {
          await addFontsLinkToHTML(htmlPath, cssFilePath)
        }
      } catch (_) {}
    ...
    outputChannel?.appendLine(`🔗 CSS linked: ${cssFilePath}`)
  } catch (error) {
...
function getStyleFilePattern(type) {
  switch (type) {
    ...
    case "custom":
      return /^.*-канвас\.css$|^макет.*\.css$|^custom.*\.css$/i
    default:
      return /\.css$/i
  }
}
...
`
    ...
    case "custom":
      return `/* Кастомні стилі під макет - ${date} */
/* Згенеровано CSS Classes from HTML v0.0.7 */

/* ========================================
   Стилі специфічні для поточного макету
======================================== */
