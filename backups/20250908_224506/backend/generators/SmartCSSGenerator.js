...
class SmartCSSGenerator {
  ...
  generateCSS(figmaData, htmlData, preMatchedElements = null) {
    ...
    console.log('🧠 Starting smart CSS generation...');
    ...
    matches.forEach((htmlElementId, figmaElementId) => {
      ...
      if (figmaElement && htmlElement) {
        this.generateElementStyles(figmaElement, htmlElement, matches.get(figmaElementId));
      }
    });
  ...
  generateElementStyles(figmaElement, htmlElement, matchInfo) {
    ...
    console.log(
      `📝 Generated styles for .${className} from "${figmaElement.name}" (${figmaElement.type})`
    );
  }
  ...
  compileCSS() {
    ...
    // ✅ FIX: Заголовок
    css += '/* ✅ CSS Generated by Smart CSS Generator v5.0 */\n';
    css += `/* Generated: ${this.statistics.generatedAt} */\n`;
    css += `/* Matched: ${this.statistics.matchedElements} | Unmatched: ${this.statistics.unmatchedElements} */\n`;
    css += `/* Accuracy: ${(this.statistics.matchingAccuracy * 100).toFixed(1)}% */\n\n`;
