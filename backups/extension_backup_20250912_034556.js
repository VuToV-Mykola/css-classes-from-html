  ...
  panel.webview.onDidReceiveMessage(async message => {
    try {
      ...
      case 'generateCSS': {
        ...
        const htmlFilePath = message.settings?.currentHTMLPath || currentHTMLFile;
        await generateResponsiveCSSFromHTML(htmlFilePath, message.settings, panel);
        break;
      }
...
// Функція для генерації Reset стилів
...
// Функція для генерації CSS змінних
...
// ✅ FIX: Головна функція генерації CSS з HTML з підтримкою responsive стилів ✅
async function generateResponsiveCSSFromHTML(htmlFilePath, settings = {}, panel = null) {
  try {
    outputChannel?.appendLine(`🚀 Розпочато генерацію CSS з HTML файлу: ${htmlFilePath}`);
    
    if (!htmlFilePath) {
      throw new Error('HTML файл не вказаний');
    }
    ...
    // Базова генерація CSS залежно від режиму
    switch (settings.mode) {
    case 'minimal':
      generatedCSS = generateBasicCSS(htmlContent, settings);
      break;
    case 'maximum':
      generatedCSS = generateBasicCSS(htmlContent, settings);
      break;
    case 'production':
      generatedCSS = generateProductionCSS(htmlContent);
      break;
    default:
      generatedCSS = generateBasicCSS(htmlContent, settings);
    }
    
    // Додаємо responsive стилі якщо потрібно
    if (settings.includeResponsive) {
      generatedCSS += '\n\n' + generateResponsiveStyles();
    }
    
    // Додаємо CSS змінні якщо потрібно
    if (settings.includeVariables) {
      generatedCSS = generateCSSVariables() + '\n\n' + generatedCSS;
    }
    
    // Додаємо Reset стилі якщо потрібно
    if (settings.includeReset) {
      generatedCSS = generateResetCSS() + '\n\n' + generatedCSS;
    }
    
    // Зберігаємо згенерований CSS
    const cssFileName = settings.cssFileName || 'styles.css';
    await saveGeneratedCSS(generatedCSS, htmlFilePath, settings);
    
    outputChannel?.appendLine(`✅ CSS успішно згенеровано: ${cssFileName}`);
    
    // Повідомляємо webview про успіх
    if (panel) {
      panel.webview.postMessage({
    ...
    const errorMessage = `❌ Помилка генерації CSS: ${error.message}`;
    outputChannel?.appendLine(errorMessage);
    
    // Повідомляємо webview про помилку
    if (panel) {
      panel.webview.postMessage({
        command: 'cssGenerated',
        success: false,
