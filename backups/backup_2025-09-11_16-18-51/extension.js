...
function activate(context) {
  ...
  try {
    ...
    try {
      ...
      outputChannel.appendLine('✅ UniversalMatchingEngine ініціалізовано - точне співставлення доступне');
    } catch (error) {
...
async function handleGenerateCSS(panel, settings) {
  try {
    ...
    // TODO: Implement loadSelectedStyleLibrary function
    try {
    ...
    if (settings.includeModernNormalize === true) {
      try {
        // TODO: Implement ensureModernNormalizeInHtml function
...
async function addModernNormalizeToHTML(htmlFilePath, settings) {
  try {
    ...
    const modernNormalizeComment = '<!--!!! MODERN NORMALIZE !!!-->';
    const modernNormalizeLink = '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/modern-normalize/1.1.0/modern-normalize.min.css" integrity="sha512-wpPYUAdjBVSE4KJnH1VR1HeZfpl1ub8YT/NKx4PuQ5NmX2tKuGu6U/JRp5y+Y8XG2tV+wKQpNHVUX03MfMFn9Q==" crossorigin="anonymous" referrerpolicy="no-referrer">';
    
    // Знаходимо позицію після тегу <title> або перед </head>
    ...
    const newHtmlContent = htmlContent.slice(0, insertPosition) + insertContent + htmlContent.slice(insertPosition);
    fs.writeFileSync(htmlFilePath, newHtmlContent, 'utf8');
    
    outputChannel?.appendLine(`✅ ${getExtensionTranslation('modernNormalizeAdded')}`);
  } catch (error) {
    outputChannel?.appendLine(`❌ ${getExtensionTranslation('modernNormalizeError')}: ${error.message}`);
  }
}
