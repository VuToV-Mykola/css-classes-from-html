...
class FontImporter {
  ...
  async importFonts(figmaClient, fileKey, selectedLayers = []) {
    try {
      console.log("🔤 Starting font analysis from Figma...")
      ...
      console.log(`🔤 Found ${figmaFonts.length} unique fonts in Figma`)
      ...
      for (const figmaFont of figmaFonts) {
        try {
          ...
          console.log(`✅ Processed font: ${result.name}`)
        } catch (error) {
          console.error(`❌ Failed to process font ${figmaFont.family}:`, error.message)
        }
      }
      ...
      return {
        ...
        imports: imports,
        cssFile: path.join(this.outputDir, "fonts.css"),
        htmlFile: path.join(this.outputDir, "font-imports.html"),
        message: `Successfully processed ${processedFonts.length} fonts`
      }
    } catch (error) {
      console.error("❌ Font import failed:", error.message)
      throw new Error(`Font import failed: ${error.message}`)
    }
  }
  ...
  saveFontFiles(imports, css) {
    ...
    // ✅ FIX: HTML файл з imports
    const htmlContent = `<!DOCTYPE html>\n<html>\n<head>\n  <meta charset="UTF-8">\n  <title>Font Imports</title>\n  \n  <!-- Google Fonts -->\n${imports.html}\n  \n  <link rel="stylesheet" href="fonts.css">\n</head>\n<body>\n  <h1>Fonts imported from Figma</h1>\n</body>\n</html>`

    const htmlPath = path.join(this.outputDir, "font-imports.html")
