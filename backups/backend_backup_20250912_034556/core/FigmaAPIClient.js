...
class FigmaAPIClient {
  ...
  /**
   * ✅ FIX: Отримання імені файлу та canvas для формування імені CSS файлу
   */
  async getFileNameAndCanvas(fileKey, canvasName) {
    try {
      const fileData = await this.getFile(fileKey);
      const fileName = fileData.name || 'Untitled';
      
      // Очищуємо імена від спецсимволів для безпечного використання в іменах файлів
      const sanitizedFileName = this.sanitizeFileName(fileName);
      const sanitizedCanvasName = this.sanitizeFileName(canvasName || 'Canvas');
      
      return {
        fileName: sanitizedFileName,
        canvasName: sanitizedCanvasName,
      ...
      return {
        ...
        fullFileName: `Untitled#${canvasName || 'Canvas'}.css`
      };
    }
  }

  /**
   * ✅ FIX: Очищення імен файлів від небезпечних символів
   */
  sanitizeFileName(name) {
    ...
    return name
      ...
      .slice(0, 200) || 'Untitled'; // Обмежуємо довжину імені
  }
