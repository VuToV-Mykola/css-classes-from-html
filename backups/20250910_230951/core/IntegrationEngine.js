...
class IntegrationEngine {
  ...
  async getFigmaCanvases(figmaFileId) {
    this.ensureClient();
    // 🛠️ ✅ FIX: Додаткове логування для діагностики проблем Canvas
    console.log(`[IntegrationEngine] getFigmaCanvases(): fileId=${figmaFileId}`);
    ...
    console.log(`[IntegrationEngine] getFigmaCanvases(): pages=${pages.length}`);
    return pages;
  }

  async getFigmaLayers(figmaFileId, canvasIds = []) {
    this.ensureClient();
    // 🛠️ ✅ FIX: Додаткове логування для діагностики проблем Layers
    console.log(`[IntegrationEngine] getFigmaLayers(): fileId=${figmaFileId}, canvasIds=${Array.isArray(canvasIds)?canvasIds.join(','):''}`);
    const file = await this.figmaClient.getFile(figmaFileId);
    ...
    console.log(`[IntegrationEngine] getFigmaLayers(): layers=${layers.length}`);
    return layers;
  }
