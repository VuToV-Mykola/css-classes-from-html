...
class ModernCSSGenerator {
  ...
  convertToLogicalProperties(styles) {
    ...
    // Маппінг фізичних властивостей на логічні
    const propertyMap = {
      // Margins
    ...
    // Конвертація властивостей
    Object.entries(propertyMap).forEach(([physical, logical]) => {
      if (physical in logical) {
        if (typeof logical === 'function') {
          Object.assign(logical, logical(logical[physical]));
        } else {
          logical[logical] = logical[physical];
        }
  ...
  compileToCSS(modernStyles) {
    ...
    // CSS змінні
    if (Object.keys(modernStyles.variables).length > 0) {
      css += ':root {\n';
    ...
    // Cascade layers
    if (modernStyles.layers && Object.keys(modernStyles.layers.styles).length > 0) {
      css += `@layer ${modernStyles.layers.order.join(', ')};\n\n`;
    ...
    // Container queries
    if (modernStyles.container && modernStyles.container.queries.length > 0) {
      css += `\n.container {\n`;
      css += `  container-type: ${modernStyles.container.type};\n`;
      css += `  container-name: ${modernStyles.container.name};\n`;
      css += '}\n';
