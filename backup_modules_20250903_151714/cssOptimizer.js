...
class CSSOptimizer {
  ...
  optimize(properties) {
    ...
    // Фільтруємо тільки валідні властивості
    const validProps = properties.filter(prop => prop && prop.name && prop.value)

    // Якщо після фільтрації порожньо – повертаємо []
    if (validProps.length === 0) return []

    // Усуваємо дублікати (останнє значення має пріоритет)
    const seen = {}
    for (let i = validProps.length - 1; i >= 0; i--) {
      if (!seen[validProps[i].name]) {
        seen[validProps[i].name] = validProps[i]
      }
    }
    ...
    rules.forEach(rule => {
      if (rule.type === "rule") {
        rule.declarations = rule.declarations.filter(decl => {
          ...
          return !rule.selectors.some(selector => this.isInheritedFromParent(selector, decl, rules))
        })
      }
    })
  ...
  stringifyCSS(rules) {
    return rules
      .map(rule => {
        if (rule.type === "rule" && rule.declarations.length > 0) {
          const declarations = rule.declarations.map(d => `  ${d.property}: ${d.value};`).join("\n")
  ...
  isInheritedFromParent(selector, declaration, rules) {
    ...
    return parentRule.declarations.some(
      d => d.property === declaration.property && d.value === declaration.value
    )
  }
  ...
  modernizeSyntax(css) {
    ...
    if (!this.modernReplacementRegexes) {
      this.modernReplacementRegexes = Object.entries(modernReplacements).map(([old, modern]) => [
