  ...
  initialize(extensionPath, storagePath) {
    ...
    try {
      ...
      outputChannel?.appendLine(
        `⚠️ Не вдалося створити теку користувацьких стилів: ${error.message}`
      )
    }
  },
    ...
    if (settings.mode === "minimal") {
      ...
      if (settings.figmaLink && settings.figmaToken) {
        ...
        if (fileId) {
          ...
          console.log(
            "🔍 ДІАГНОСТИКА: Figma Data Structure:",
            JSON.stringify(figmaData, null, 2).substring(0, 1000) + "..."
          )
          console.log(
            "🔍 ДІАГНОСТИКА: HTML Data Structure:",
            JSON.stringify(htmlData, null, 2).substring(0, 1000) + "..."
          )
          ...
          await fs.writeFile(
            path.join(debugDir, `figma-data-${timestamp}.json`),
            JSON.stringify(figmaData, null, 2)
          )
          await fs.writeFile(
            path.join(debugDir, `html-data-${timestamp}.json`),
            JSON.stringify(htmlData, null, 2)
          )

          outputChannel?.appendLine("🔍 Debug files saved to debugs/ directory")
          outputChannel?.appendLine(
            `📊 Figma nodes available: ${figmaData?.document?.children?.length || 0}`
          )
          ...
          const matchingResults = await matchingEngine.match(
          ...
          outputChannel?.appendLine(
            `✅ CSS згенеровано з ${matchingResults.length} співставленнями`
          )
          ...
          try {
            ...
            if (FontImporter) {
              ...
              if (fontResult && fontResult.imports && fontResult.imports.html) {
                ...
                outputChannel?.appendLine(
                  `✅ Google Fonts автоматично додано: ${fontStats.totalFonts} шрифтів`
                )
              }
            } else {
              outputChannel?.appendLine(
                "⚠️ FontImporter не доступний для автоматичного імпорту шрифтів"
              )
            }
          } catch (fontError) {
            outputChannel?.appendLine(
              `⚠️ Помилка автоматичного імпорту шрифтів: ${fontError.message}`
            )
          }
          ...
          outputChannel?.appendLine(
            `   • Unmatched elements: ${(htmlData.elements || []).length - matchingResults.length}`
          )
    ...
    if (
      ...
      try {
        ...
        if (fileId && integrationEngine.figmaClient) {
          ...
          const selectedCanvasIds = Array.isArray(settings.selectedCanvases)
            ? settings.selectedCanvases
            : []
          ...
          try {
            ...
            if (selectedCanvasIds.length > 0) {
              ...
              if (canvas && canvas.name) {
                ...
                outputChannel?.appendLine(
                  "⚠️ Вибраний Canvas не знайдено, використовуємо перший доступний"
                )
              }
            }
          ...
          const fileNameInfo = await integrationEngine.figmaClient.getFileNameAndCanvas(
            fileId,
            canvasName
          )

          // Зберігаємо в користувацьку теку
          const saveResult = await userStylesManager.saveUserStyle(fileNameInfo.fullFileName, css, {
          ...
          if (saveResult.success) {
            userStyleSaved = true
            outputChannel?.appendLine(
              `💾 CSS збережено як користувацький стиль: ${fileNameInfo.fullFileName}`
            )
          }
        }
      } catch (error) {
...
async function ensureModernNormalizeInHtml(htmlFilePath) {
  try {
    ...
    if (!existingLink) {
      ...
      // ✅ FIX: Вставляємо normalize після всіх meta тегів
      const metaTags = head.querySelectorAll("meta")
      const normalizeComment = document.createComment("!!! normalize !!!")

      if (metaTags.length > 0) {
        // Знаходимо останній meta тег
        const lastMetaTag = metaTags[metaTags.length - 1]
        // Вставляємо після останнього meta тегу
        lastMetaTag.parentNode.insertBefore(normalizeComment, lastMetaTag.nextSibling)
        lastMetaTag.parentNode.insertBefore(link, normalizeComment.nextSibling)
      } else {
        // Якщо немає meta тегів, додаємо на початок head
...
async function addModernNormalizeToHTML(htmlFilePath, settings) {
  try {
    ...
    // ✅ FIX: Вставляємо normalize після всіх meta тегів з відступом 4 пробіли
    const openHead = htmlContent.match(/<head[^>]*>/i)
    const headMatch = htmlContent.match(/<\/head>/i)

    if (!openHead) {
      outputChannel?.appendLine("⚠️ Could not find <head> tag in HTML file")
      return
    }

    // Знаходимо всі meta теги в head
    const headStart = openHead.index + openHead[0].length
    const headEnd = headMatch ? headMatch.index : htmlContent.length
    const headContent = htmlContent.slice(headStart, headEnd)

    // Шукаємо останній meta тег
    const metaTags = headContent.match(/<meta[^>]*>/gi)
    let insertPosition
    let insertContent

    if (metaTags && metaTags.length > 0) {
      // Знаходимо позицію після останнього meta тегу
      const lastMetaTag = metaTags[metaTags.length - 1]
      const lastMetaIndex = headContent.lastIndexOf(lastMetaTag)
      const lastMetaEnd = headStart + lastMetaIndex + lastMetaTag.length

      // Вставляємо після останнього meta тегу з відступом 4 пробіли
      insertPosition = lastMetaEnd
      insertContent = `\n    ${modernNormalizeComment}\n    ${modernNormalizeLink}`
    } else {
      // Якщо немає meta тегів, вставляємо після <head> з відступом 4 пробіли
...
async function handleImportFonts(panel, message) {
  try {
    ...
    outputChannel?.appendLine(
      `🔍 Debug: result.imports = ${JSON.stringify(result.imports, null, 2)}`
    )
    outputChannel?.appendLine(
      `🔍 Debug: result.imports.html length = ${result.imports?.html?.length || 0}`
    )
...
<body>
    ...
    outputChannel?.appendLine(
      `✅ Fonts imported: ${stats.totalFonts} fonts - імпорти додані в HTML`
    )
...
async function handleGenerateAutoSaveFileName(panel, message) {
  try {
    ...
    try {
      ...
      if (selectedCanvasIds.length > 0) {
        ...
        if (canvas && canvas.name) {
          canvasName = canvas.name
          outputChannel?.appendLine(
            `✅ Використовуємо вибраний Canvas для автозбереження: ${canvasName}`
          )
        } else {
          outputChannel?.appendLine(
            "⚠️ Вибраний Canvas не знайдено, використовуємо перший доступний"
      ...
      if (canvasName === "Canvas" && figmaData?.document?.children?.length > 0) {
        ...
        if (firstCanvas && firstCanvas.name) {
          canvasName = firstCanvas.name
          outputChannel?.appendLine(
            `✅ Використовуємо перший Canvas для автозбереження: ${canvasName}`
          )
        }
      }
    } catch (e) {
    ...
    outputChannel?.appendLine(
      `✅ Згенеровано ім'я файлу для автозбереження: ${fileName.fullFileName}`
    )
  } catch (error) {
...
async function handleRenameLayer(panel, message) {
  try {
    ...
    if (figmaLink && figmaToken && userStylesManager) {
      try {
        outputChannel?.appendLine(
          `🔤 Автоматично зберігаю стилі для переіменованого Layer: ${newName}`
        )
...
${style.css || style.styles || "  /* Стилі Layer'а */"}
            ...
            // Генеруємо ім'я файлу
            const fileName = await integrationEngine.figmaClient.getFileNameAndCanvas(
              fileId,
              canvasName
            )
            ...
            if (saveResult.success) {
              outputChannel?.appendLine(
                `✅ Стилі Layer'а "${newName}" автоматично збережено в ${fileName.fullFileName}`
              )
            } else {
