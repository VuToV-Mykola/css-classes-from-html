...
function generateCSS(classes, classDictionary, includeGlobal = true, includeReset = true, designTokens = null, selectedTags = null, options = {}) {
  ...
  if (options.optimizeCSS) {
    ...
    css = optimizer.optimizeCSS(css, {
      ...
      fixDeprecated: options.fixDeprecated !== false,
      fixNumberPrecision: options.fixNumberPrecision !== false,
      removeDuplicateSelectors: options.removeDuplicateSelectors !== false
    })
  }
  // Автофікс Stylelint для специфічних правил
  if (options.autoFixStylelint !== false) {
    const optimizer2 = new CSSOptimizer()
    css = optimizer2.autoFixStylelint(css)
  }
