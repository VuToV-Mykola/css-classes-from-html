...
class UniversalMatchingEngine {
  ...
  async match(figmaData, htmlData) {
    ...
    try {
      ...
      const hierarchicalMatches = await this.findHierarchicalMatches(normalizedFigma, normalizedHtml);
      console.log(`🌳 Ієрархічних співпадінь: ${hierarchicalMatches.length}`);
      ...
      const hierarchyMatches = await this.findHierarchyMatches(
        normalizedFigma, 
        normalizedHtml, 
        [...hierarchicalMatches, ...textMatches]
      );
      console.log(`🌳 Додаткових ієрархічних співпадінь: ${hierarchyMatches.length}`);
      
      // Етап 3: Семантичний аналіз
      const semanticMatches = await this.findSemanticMatches(
        normalizedFigma, 
        normalizedHtml, 
        [...hierarchicalMatches, ...textMatches, ...hierarchyMatches]
      );
      ...
      const styleMatches = await this.findStyleMatches(
        normalizedFigma, 
        normalizedHtml, 
        [...hierarchicalMatches, ...textMatches, ...hierarchyMatches, ...semanticMatches]
      );
  ...
  async findHierarchicalMatches(figmaNodes, htmlElements) {
    ...
    console.log('🌳 Початок ієрархічного аналізу...');
    
    // ✅ FIX: Знаходимо головний вузол Figma (найбільший FRAME)
    const mainFigmaNode = this.findMainFigmaNode(figmaNodes);
    if (!mainFigmaNode) {
      console.log('❌ Головний вузол Figma не знайдено');
      return matches;
    }
    
    console.log(`🎯 Головний вузол Figma: ${mainFigmaNode.name} (${mainFigmaNode.type})`);
    console.log(`📊 Дочірніх вузлів: ${mainFigmaNode.children ? mainFigmaNode.children.length : 0}`);
    
    // ✅ FIX: Знаходимо body елемент в HTML
    const bodyElement = this.findBodyElement(htmlElements);
    if (!bodyElement) {
      console.log('❌ Body елемент не знайдено');
      return matches;
    }
    
    console.log(`🎯 Body елемент: ${bodyElement.tagName}`);
    console.log(`📊 Дочірніх елементів: ${bodyElement.children ? bodyElement.children.length : 0}`);
    ...
    console.log(`✅ Головний вузол співставлено з body (100%)`);
    ...
    console.log(`✅ Дочірніх співпадінь: ${childMatches.length}`);
  ...
  async matchChildrenHierarchically(figmaChildren, htmlChildren, parentFigma, parentHtml) {
    ...
    console.log(`🔍 Співставлення дочірніх елементів:`);
    console.log(`   Figma: ${figmaChildren.length} вузлів`);
    ...
    console.log(`📊 Схожість кількості: ${(countSimilarity * 100).toFixed(1)}%`);
  ...
  async createHierarchicalMatch(figmaNode, htmlElement, parentFigma, parentHtml, index) {
    const confidence = this.calculateHierarchicalConfidence(figmaNode, htmlElement, parentFigma, parentHtml, index);
    
    if (confidence >= this.options.thresholds.low) {
      console.log(`✅ Співставлення: ${figmaNode.name || figmaNode.type} ↔ ${htmlElement.tagName}.${htmlElement.className || 'no-class'} (${(confidence * 100).toFixed(1)}%)`);
  ...
  async findTextMatches(figmaNodes, htmlElements) {
    ...
    console.log('🎯 Початок пошуку текстових співпадінь...');
    console.log(`📊 Figma текстових вузлів: ${figmaNodes.filter(n => n.type === 'TEXT' && n.characters).length}`);
    ...
    for (const figmaNode of figmaNodes) {
      if (figmaNode.type === 'TEXT' && figmaNode.characters) {
        console.log(`🔍 Шукаємо співпадіння для Figma тексту: "${figmaNode.characters}"`);
        ...
        if (bestMatch && bestMatch.confidence < 1.0) {
          console.log(`🎯 Найкраще співпадіння: ${(bestMatch.confidence * 100).toFixed(1)}%`);
          console.log(`   Figma: "${bestMatch.metadata.figmaText}"`);
          console.log(`   HTML:  "${bestMatch.metadata.htmlText}"`);
    ...
    console.log(`✅ Знайдено ${matches.length} текстових співпадінь`);
    console.log(`🎯 100% точних співпадінь: ${matches.filter(m => m.confidence === 1.0).length}`);
  ...
  calculateTextSimilarity(figmaText, htmlText) {
    ...
    if (normalizedHtml.includes(normalizedFigma) && normalizedFigma.length > 3) {
      console.log(`🎯 HTML містить Figma текст: "${normalizedHtml}" contains "${normalizedFigma}"`);
      return 0.95;
    }
    ...
    if (normalizedFigma.includes(normalizedHtml) && normalizedHtml.length > 3) {
      console.log(`🎯 Figma містить HTML текст: "${normalizedFigma}" contains "${normalizedHtml}"`);
      return 0.95;
    }
      ...
      for (const htmlWord of htmlWords) {
        ...
        if (figmaWord.length > 2 && htmlWord.length > 2) {
          if (figmaWord.includes(htmlWord) || htmlWord.includes(figmaWord)) {
            ...
            console.log(`🎯 Частина слова знайдена: "${figmaWord}" ~ "${htmlWord}"`);
            break;
          }
        }
            ...
            for (const htmlPart of htmlParts) {
              if (figmaPart.length > 2 && htmlPart.length > 2) {
                if (figmaPart.includes(htmlPart) || htmlPart.includes(figmaPart)) {
                  ...
                  console.log(`🎯 Частина класу знайдена: "${figmaPart}" ~ "${htmlPart}"`);
                  break;
                }
              }
            }
    ...
    if (totalWords > 0) {
      ...
      if (wordSimilarity >= 0.4) { // ✅ FIX: Знижуємо поріг до 40%
        console.log(`🎯 Співпадіння слів: ${(wordSimilarity * 100).toFixed(1)}%`);
        console.log(`   Figma слова: [${figmaWords.join(', ')}]`);
        console.log(`   HTML слова: [${htmlWords.join(', ')}]`);
        console.log(`   Спільні слова: ${commonWords}/${totalWords}`);
    ...
    if (similarity > 0.8) {
      console.log(`🎯 Високе співпадіння: "${normalizedFigma}" ~ "${normalizedHtml}" (${(similarity * 100).toFixed(1)}%)`);
    }
  ...
  getHtmlElementPriority(tagName) {
    const priority = {
      ...
      'P': 6, 'SPAN': 7, 'DIV': 8, 'SECTION': 9, 'ARTICLE': 10,
      'HEADER': 11, 'FOOTER': 12, 'NAV': 13, 'MAIN': 14,
      'IMG': 15, 'A': 16, 'BUTTON': 17, 'INPUT': 18
    };
