...
class AdvancedCSSGenerator {
  ...
  /**
   * ✅ FIX: Генерація CSS з використанням AdvancedMatchingEngine
   */
  async generateCSS(figmaData, htmlData, matchingResults) {
    console.log('🎨 Початок розширеної генерації CSS...');
    ...
    // Обробляємо кожне співставлення
    for (const match of matchingResults) {
      await this.generateCSSForMatch(match);
    }
    
    // Генеруємо фінальний CSS
  ...
  compileCSS() {
    ...
    for (const selector of sortedSelectors) {
      ...
      const { rules, confidence, source, canvas, layer, metadata } = ruleData;
      
      if (Object.keys(rules).length > 0) {
        // ✅ FIX: Додаємо коментар з інформацією про Canvas та Layer
        if (this.options.includeComments) {
          const layerLabel = layer.originalName && layer.originalName !== layer.name
            ? `${layer.name} (alias for: ${layer.originalName})`
            : layer.name;
          css += `/* Canvas: ${canvas.name} | Layer: ${layerLabel} | Source: ${source} */\n`;
        }
