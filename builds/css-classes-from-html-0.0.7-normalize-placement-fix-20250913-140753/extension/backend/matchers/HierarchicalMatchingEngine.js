...
class HierarchicalMatchingEngine {
  ...
  async match(figmaNodes, htmlElements, figmaData = null) {
    logger.info("🎯 Початок ієрархічного співставлення...");
    logger.info(`📊 Figma вузлів: ${figmaNodes ? figmaNodes.length : 0}`);
    logger.info(`📊 HTML елементів: ${htmlElements ? htmlElements.length : 0}`);
    ...
    logger.info("🔍 ДІАГНОСТИКА figmaNodes:");
    if (figmaNodes && figmaNodes.length > 0) {
      for (let i = 0; i < Math.min(5, figmaNodes.length); i++) {
        const node = figmaNodes[i];
        logger.info(`  [${i}] ID: ${node.id}, Type: ${node.type}, Name: "${node.name}", Children: ${node.children?.length || 0}`);
      }
    } else {
      logger.info("  ❌ figmaNodes порожній або не визначений");
    }

    logger.info("🔍 ДІАГНОСТИКА htmlElements:");
    if (htmlElements && htmlElements.length > 0) {
      for (let i = 0; i < Math.min(5, htmlElements.length); i++) {
        const element = htmlElements[i];
        logger.info(`  [${i}] Tag: ${element.tagName}, Classes: [${element.classes?.join(", ") || ""}], Text: "${element.textContent?.substring(0, 50) || ""}"`);
      }
    } else {
      logger.info("  ❌ htmlElements порожній або не визначений");
    }

    if (!figmaNodes || !figmaNodes.length || !htmlElements || !htmlElements.length) {
      logger.warn("⚠️ Порожній набір вузлів або елементів");
      return [];
    }
    ...
    logger.info(`🌳 Кореневий Figma вузол: ${rootFigmaNode?.name || "не знайдено"}`);
    logger.info(`🌳 Кореневий HTML елемент: ${rootHtmlElement?.tagName || "не знайдено"}`);
    ...
    logger.info(`✅ Загальна кількість співставлень: ${matches.length}`);
  ...
  async performHierarchicalMatching(rootFigmaNode, rootHtmlElement, figmaNodes, htmlElements) {
    ...
    logger.info(`✅ Кореневе співставлення: ${rootFigmaNode.name} ↔ ${rootHtmlElement.tagName}`);
  ...
  async matchChildren(figmaParent, htmlParent, figmaNodes, htmlElements, matches, usedFigmaNodes, usedHtmlElements, level) {
    ...
    logger.info(`📊 Рівень ${level}: Figma дітей ${figmaChildren.length}, HTML дітей ${htmlChildren.length}`);
    
    // Стратегія 1: Співставлення по позиції (якщо кількість однакова)
    if (figmaChildren.length === htmlChildren.length) {
      logger.info("🎯 Позиційне співставлення (однакова кількість дітей)");
      ...
      for (let i = 0; i < figmaChildren.length; i++) {
        ...
        logger.info(`  ↳ ${figmaChild.name || figmaChild.type} ↔ ${htmlChild.tagName}[${i}]`);
      ...
      logger.info("🧠 Розумне співставлення (різна кількість дітей)");
      ...
      for (const figmaChild of unmatchedFigmaChildren) {
        ...
        if (bestHtmlMatch.element && bestHtmlMatch.confidence >= this.options.confidence.minimal) {
          ...
          logger.info(`  ↳ ${figmaChild.name || figmaChild.type} ↔ ${bestHtmlMatch.element.tagName} (${bestHtmlMatch.reason})`);
  ...
  async performTextMatching(figmaNodes, htmlElements, existingMatches) {
    ...
    logger.info("📝 Початок текстового співставлення...");
    ...
    for (const figmaNode of textFigmaNodes) {
      ...
      for (const htmlElement of htmlElements) {
        ...
        if (htmlText && this.isTextMatch(figmaText, htmlText)) {
          ...
          logger.info(`  ✅ Текстове співпадіння: "${figmaText.substring(0, 30)}..." ↔ ${htmlElement.tagName}`);
          break;
        }
      }
    }
    
    logger.info(`📝 Текстових співпадінь знайдено: ${matches.length}`);
    return matches;
  }
  ...
  async performClassNameMatching(figmaNodes, htmlElements, existingMatches) {
    ...
    logger.info("🏷️ Початок співставлення за назвами класів...");
      ...
      for (const htmlElement of htmlElements) {
        ...
        for (const htmlClass of htmlClasses) {
          ...
          if (this.isClassNameMatch(figmaName, normalizedHtmlClass)) {
            ...
            logger.info(`  🎯 Клас співпадіння: "${figmaNode.name}" ↔ ".${htmlClass}"`);
            break;
          }
        }
    ...
    logger.info(`🏷️ Співпадінь за класами знайдено: ${matches.length}`);
    return matches;
  }
  ...
  async performStructuralMatching(figmaNodes, htmlElements, existingMatches) {
    ...
    logger.info("🏗️ Початок структурного співставлення...");
    ...
    for (const figmaNode of unmatchedFigmaNodes) {
      ...
      if (bestMatch.element) {
        ...
        logger.info(`  🏗️ Структурне співпадіння: ${figmaNode.name || figmaNode.type} ↔ ${bestMatch.element.tagName}`);
      }
    }
    
    logger.info(`🏗️ Структурних співпадінь знайдено: ${matches.length}`);
    return matches;
  }
  ...
  deduplicateMatches(matches) {
    ...
    logger.info(`🔄 Після дедуплікації: ${uniqueMatches.length} унікальних співставлень`);
