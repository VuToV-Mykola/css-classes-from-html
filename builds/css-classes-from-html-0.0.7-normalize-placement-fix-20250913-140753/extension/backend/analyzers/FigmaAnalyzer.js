...
class FigmaAnalyzer {
  constructor() {
    this.analysis = {
      ...
      semantics: new Map(),
      content: new Map(),
      contentMap: new Map(), // normalizedText -> elementInfo;
      structure: {
        depth: 0,
        totalElements: 0,
    ...
    // ✅ FIX: Ініціалізація UniversalMatchingEngine;
    this.matchingEngine = null;
  }

  /**
   * ✅ FIX: Встановлення UniversalMatchingEngine;
   */
  ...
  /**
   * ✅ FIX: Точне співставлення Figma елементів з HTML;
   */
  async matchWithHTML(figmaData, htmlData) {
    if (!this.matchingEngine) {
      throw new Error("UniversalMatchingEngine не встановлено");
    }
  ...
  // Безпечні заглушки/узагальнення для відсутніх методів
  extractPageStyles(page) {
    const layout = {};
    if (page.layoutMode) {
      layout.display = page.layoutMode === "HORIZONTAL" ? "flex" : "block";
      layout.flexDirection = page.layoutMode === "HORIZONTAL" ? "row" : "column";
    }
    return { layout };
  }

  analyzeStyles(figmaData) {
    // Узагальнення: стилі вже збираються у extractElementStyles під час обходу
    return this.analysis.styles;
  }

  analyzeSemantics(_figmaData) {
    // Узагальнення: семантика визначається у determineElementSemanticRole/ determinePageSemanticRole;
    return this.analysis.semantics;
  }

  analyzeContent(_figmaData) {
    // Узагальнення: контент фіксується у extractElementContent;
  ...
  extractElementContent(element) {
    ...
    // Індекс для контентного матчінгу
    if (content.hasText && content.text) {
      const key = this.normalizeText(content.text);
      if (key) {
        // елемент-носій інформації для матчінгу
        const elementInfo = this.analysis.hierarchy.get(element.id) || {
          id: element.id,
          name: element.name || "Unnamed",
          type: element.type;
        };
        this.analysis.contentMap.set(key, elementInfo);
      }
    }
    return content;
  }
